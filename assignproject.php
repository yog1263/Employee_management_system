...
$sql = "SELECT * FROM project WHERE status='Due'";
...
<body>
	...
	<div class="divider"></div>
		...
		<table>
			...
			<?php
				...
				while ($employee = mysqli_fetch_assoc($result)) {
					$sql1 = "SELECT firstName,lastName from `employee` WHERE id= ".$employee['eid']." ";
					...
					while ($employee1 = mysqli_fetch_assoc($result1)) 
					{
						...
						echo "<td>".$employee1['firstName']." ".$employee1['lastName']."</td>";
		...
		<table>
			...
			<?php
			$sql = "SELECT * from project WHERE status='Submitted' order by subdate desc";
			//echo "$sql";
			$result = mysqli_query($conn, $sql);
				while ($employee = mysqli_fetch_assoc($result)) {
					$sql1 = "SELECT firstName,lastName from `employee` WHERE id= ".$employee['eid']." ";
